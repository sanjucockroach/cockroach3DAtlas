...
/**
 * CockroachIAS — UPSC World Atlas — left FilterPanel
 * Layer toggles + syllabus lens + live-status filter + legend.
 */
...
export default function FilterPanel({ hotspots }: Props) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* layers */}
      <div className="ci-scroll flex-1 overflow-y-auto px-2 py-2">
        {LAYERS.map((layer) => {
      ...
      <div className="border-t border-[#e0e0e0] px-4 py-3">
        ...
        <div className="ci-rule mt-2 w-8" />
        <ul className="mt-2 space-y-1.5 text-[11px] text-[#444]">
          <li className="flex items-center gap-2">
            <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#d32f2f]" />
            Red — alerts, IR, conflict, straits
          </li>
          <li className="flex items-center gap-2">
            <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#1565c0]" />
            Blue — water: rivers, Ramsar, ice
          </li>
          <li className="flex items-center gap-2">
            <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#2e7d32]" />
            Green — life: biodiversity, heritage, minerals
          </li>
          <li className="flex items-center gap-2 pt-1">
            <span className="inline-block h-3 w-3 bg-black ring-1 ring-white" />
            Land mass
          </li>
          <li className="flex items-center gap-2">
            <span className="inline-block h-3 w-3 bg-white ring-1 ring-black/20" />
            Ocean / page
          </li>
        </ul>
      </div>
